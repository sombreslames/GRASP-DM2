...
for i in eachindex(FileList)
   ...
   if BPP.NBvariables <= 500 && BPP.NBconstraints <= 500 && nbProb <= 6
      ...
      if nbProb%2 == 1
         ...
         @time nothing,CS     = SimulatedAnnealing(CS,500.0,0.95,150,1.0)
         println("Simulated annealing result :",CS.CurrentObjectiveValue)
      else
         ...
         @time for k in 1:1:itmax1
            for j in 1:1:itmax
               ...
               #tempv          = CS.CurrentObjectiveValue
               CS             = GetRandomNeighbour(CS)
               #=if CS.CurrentObjectiveValue > tempv
                  println(tempv," to ",CS.CurrentObjectiveValue)
               end=#
               #Let s do it with a greedy local search
               #nothing,CS     = SimulatedAnnealing(CS,150.0,0.95,150,1.0)
